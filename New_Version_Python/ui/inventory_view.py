...
class InventoryView(ctk.CTkFrame):
    def __init__(self, parent, db: DatabaseRepository, user: User):
        ...
        # Bulk Edit Toggle -> Now "Show Checkboxes" or "Clear Selection" or "Select All"
        # Since clicking rows works now, maybe just "Select All"
        self.btn_bulk = ctk.CTkButton(self.header_frame, text="Select All", width=100, command=self.select_all_event, 
    ...
    def toggle_selection_mode(self):
        # Deprecated: Now just toggles Checkboxes Visuals if user really wants them,
        # otherwise selection is native.
        self.selection_mode = not self.selection_mode
        self.load_data() # Refresh list to show/hide checkboxes

    def clear_selection(self):
        self.selected_ids.clear()
        self.selection_mode = False # Reset mode too
        self.load_data() 
    ...
    def update_bulk_bar(self):
        # Show bar if ANY items are selected, regardless of "mode"
        ...
        if has_selection:
            ...
            # Update Header Button to "Clear"
            self.btn_bulk.configure(text="Cancel Select", fg_color=Colors.DANGER, command=self.clear_selection)
        else:
            self.bulk_bar.place_forget()
            # Reset Header Button
            self.btn_bulk.configure(text="Select All", fg_color=Colors.BG_CARD, command=self.select_all_event)
    ...
    def handle_row_click(self, event, index, product):
        ...
        if shift_pressed and self.last_clicked_index != -1:
             ...
             # Standard Click: Select Single (Clear others) and Highlight
             self.selected_ids.clear()
             self.selected_ids.add(product.product_id)
             
             # If check mode is explicitly on, toggle instead? 
             # No, standard OS behavior: Click = Select Single. Ctrl+Click = Toggle.
    ...
    def print_selected_labels(self):
        ...
        if not products_to_print:
            messagebox.showwarning("Error", "No valid barcodes found in selection.")
            return

        # Generate Sheet
        pdf = generator.generate_sheet(products_to_print)
        
        if pdf and print_file(pdf):
            msg = f"Sent {len(products_to_print)} barcodes to printer (Grid Sheet)."
            if failed > 0:
                msg += f"\nSkipped {failed} invalid items."
            messagebox.showinfo("Success", msg)
        else:
            messagebox.showerror("Error", "Failed to print barcode sheet.")
