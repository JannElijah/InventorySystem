...
class StockInView(ctk.CTkFrame):
    def __init__(self, parent, db, user):
        ...
        widths = [200, 100, 100, 100, 100, 60]
        
        for i, h in enumerate(headers):
            anchor = "w" if i < 5 else "center"
            ctk.CTkLabel(h_frame, text=h, font=("Arial", 12, "bold"), width=widths[i], anchor=anchor, text_color=Colors.TEXT_PRIMARY).pack(side="left", padx=5)
    ...
    def update_cart_ui(self):
        ...
        for item in self.cart:
            ...
            # Name
            ctk.CTkLabel(row, text=p.description, width=widths[0], anchor="w", text_color=Colors.TEXT_PRIMARY).pack(side="left", padx=5)
            # Stock
            ctk.CTkLabel(row, text=str(p.stock_quantity), width=widths[1], text_color=Colors.TEXT_SECONDARY).pack(side="left", padx=5)
            
            # Qty (Entry)
            q_frame = ctk.CTkFrame(row, fg_color="transparent", width=widths[2])
            q_frame.pack(side="left", padx=5)
            # Ensure buttons work and are visible
            btn_minus = ctk.CTkButton(q_frame, text="-", width=25, height=25, fg_color=Colors.BG_CARD, text_color=Colors.TEXT_PRIMARY,
                                      command=lambda i=item: self.update_qty(i, -1))
            btn_minus.pack(side="left", padx=2)
            
            # Edit Entry
            entry_qty = ctk.CTkEntry(q_frame, width=40, height=25, justify="center", fg_color=Colors.BG_INPUT, text_color=Colors.TEXT_INPUT, border_color=Colors.BORDER)
            ...
            btn_plus = ctk.CTkButton(q_frame, text="+", width=25, height=25, fg_color=Colors.BG_CARD, text_color=Colors.TEXT_PRIMARY,
                                     command=lambda i=item: self.update_qty(i, 1))
            btn_plus.pack(side="left", padx=2)
            
            # Cost
            ctk.CTkLabel(row, text=f"${c:.2f}", width=widths[3], text_color=Colors.TEXT_PRIMARY).pack(side="left", padx=5)
            
            # Total (Label ref needed for live update)
            lbl_total_row = ctk.CTkLabel(row, text=f"${subtotal:.2f}", width=widths[4], text_color=Colors.TEXT_PRIMARY)
            lbl_total_row.pack(side="left", padx=5)
            ...
            # Remove
            ctk.CTkButton(row, text="x", width=30, fg_color=Colors.DANGER, 
                          command=lambda i=item: self.remove_item(i)).pack(side="left", padx=5)
    ...
    def process_transaction(self):
        print("DEBUG: Process Transaction Clicked")
        if not self.cart:
